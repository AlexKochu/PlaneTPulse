...
export default function LogActivityPage() {
  ...
  const liveValidation = validateActivityInput(activityType, quantity, date);
  const isInvalid = activityType && quantity !== "" && !liveValidation.valid && !liveValidation.requiresConfirmation;
  const liveError = isInvalid ? liveValidation.error : "";

  const co2Result =
    config && !isNaN(quantityNum) && quantityNum > 0 && !isInvalid
  ...
  return (
    <motion.div className="log-page-wrapper" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.2 }}>
      <div className="page-header">
        ...
        <div className="log-preview-column">
          <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.25, delay: 0.1 }}>
            <div className="sticky-preview-wrapper">
              {isInvalid && liveError ? (
                <div className="card log-preview-placeholder error-placeholder" style={{ borderColor: '#DC2626', backgroundColor: 'rgba(239, 68, 68, 0.05)' }}>
                  <div className="placeholder-icon" style={{ color: '#DC2626', backgroundColor: 'rgba(239, 68, 68, 0.1)' }}>
                    <AlertTriangle size={36} />
                  </div>
                  <h3 className="placeholder-title" style={{ color: '#DC2626' }}>Invalid Input</h3>
                  <p className="placeholder-desc" style={{ color: 'var(--color-text-secondary)' }}>
                    {liveError}
                  </p>
                </div>
              ) : co2Result ? (
